...
class TcpMessenger:
    ...
    def send_utf(self, unicode_str):
        self.server_socket.send(struct.pack("!H", len(unicode_str)))
        self.server_socket.send(unicode_str)

    def send_string(self, normal_str):
        self.send_utf(normal_str.encode("utf"))

    def send_utfs(self, unicode_strings):
        self.send_int(len(unicode_strings))
        for unicode_str in unicode_strings:
            self.send_utf(unicode_str)

    def send_strings(self, normal_strings):
        self.send_int(len(normal_strings))
        for normal_string in normal_strings:
            self.send_string(normal_string)
